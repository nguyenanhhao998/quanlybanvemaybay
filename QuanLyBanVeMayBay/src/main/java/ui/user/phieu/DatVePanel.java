...
public class DatVePanel extends javax.swing.JPanel {
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        for(int i = 0; i < ves.size(); i++){
            VechuyenbayBUS.updateStatus(ves.get(i),"Đã được đặt");
        }
        /*Khachhang khachhang, Date ngayDat, String tinhTrang, Vechuyenbay vechuyenbay*/
        //insert x forms 
        boolean checkDatCho = true;
        for(int i = 0; i < sl; i++){
            Phieudatcho phieudc = new Phieudatcho(KhachhangBUS.getKHbyID(makh),new Date(),"Còn hiệu lực", ves.get(i));
            int maphieu = PhieudatchoBUS.insert(phieudc);
            if(maphieu == 0)
                checkDatCho = false;
        }
        if(checkDatCho){
            JLabel label = new JLabel("Đặt vé thành công.");
            label.setFont(new Font("Arial", Font.BOLD, 18));
